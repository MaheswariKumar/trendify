  ...
  <circle class="loader-path" cx="50" cy="50" r="20" fill="none" stroke="#70c542" stroke-width="4" />
        ...
        <div key={data.id} className="w-64 max-sm:w-[9rem] max-lg:w-[18rem] bg-pink-200 border border-gray-200 rounded-lg shadow-lg overflow-hidden flex flex-col">
            ...
            <div className="flex-1 p-4 flex flex-col justify-between">
                ...
                {itemCount[data.id] > 0 ? <div className="flex items-center gap-2 justify-between">
                    <button onClick={()=> navigate("/viewcart")} className="px-4 max-sm:text-[12px] max-sm:p-1 py-2 bg-[#113155] text-white font-semibold rounded hover:bg-blue-600 transition duration-300 mt-auto">Go to Cart</button>
                    <div className="flex gap-1">
                        <button className="bg-[#113155] px-3 max-sm:px-1 text-white" onClick={() => handleDecButton(data.id)}>-</button>
                        <nav className="text-xl max-sm:text-sm">{itemCount[data.id]}</nav>
                        <button className="bg-[#113155] px-3 text-white max-sm:px-1" onClick={() => {handleIncButton(data.id); handleShowPopUp()}}>+</button>
                    </div>
                    </div> : 
                <button onClick={() => {handleAddtoCart(data.id); handleShowPopUp()}} className="w-full py-2 bg-[#113155] text-white max-sm:text-sm font-semibold rounded hover:bg-blue-600 transition duration-300 mt-auto">Add to Cart</button>}
            </div>
